...
|---|---|---|
| Sprint 0 | Contract API dan schema, CI frontend/backend, Docker Compose, empat council OASIS, hard limit, unique trace, deterministic finance tool, multi-provider Gemini/OpenAI, serta test dependency | Benchmark live kedua provider setelah API key tersedia; evidence source/license decision |
...
| Sprint 3 | Product, transaksi manual, dashboard, gate tujuh hari, analytics dan insight deterministik, OCR upload/review/confirm, private object storage, PDF export, serta retention job | E2E receipt lintas stack dengan OCR, worker, database, dan object storage nyata |
| Sprint 4 | Unit, integration, contract, RBAC, finance/scoring golden test, fallback OASIS, matriks Chromium/Firefox/WebKit, accessibility audit awal, security configuration, dependency audit, serta otomatisasi migrasi, backup/restore, dan readiness load smoke | E2E receipt lintas stack, failure injection, penyelesaian advisory dependency worker OASIS, calibration, usability, ablation, dan release evidence |
